...
prezzi: [
    ...
    // Riferimento: Listino Finstral EUR 2025/10 (verificato su conferma ordine Ruggeri)
    // 
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    supplementiTelaio: {
"961": { pvcA: 0, pvcB: 5.20, aluA: 17.2, aluB: 20.4 },
"962": { pvcA: 0, pvcB: 5.61, aluA: 17.2, aluB: 22.4 },
"963": { pvcA: 2.89, pvcB: 9.31, aluA: 18.0, aluB: 24.9 },
"964": { pvcA: 5.00, pvcB: 10.5, aluA: 15.0, aluB: 17.3 },
"965": { pvcA: 7.71, pvcB: 13.1, aluA: 15.0, aluB: 17.3 },  // ✅ Verificato conferma Ruggeri
"966": { pvcA: 11.8, pvcB: 17.7, aluA: 15.0, aluB: 17.3 },
"967": { pvcA: 7.22, pvcB: 12.8, aluA: 15.0, aluB: 17.5 },
"Z62": { pvcA: 5.32, pvcB: 10.6, aluA: 15.0, aluB: 18.3 },
"Z91": { pvcA: 16.4, pvcB: 22.2, aluA: 15.0, aluB: 18.3 },
"924": { pvcA: 8.83, pvcB: 14.3, aluA: 18.0, aluB: 21.3 },
"991": { pvcA: 10.9, pvcB: 16.5, aluA: 18.0, aluB: 21.3 },
"951": { pvcA: 13.0, pvcB: 20.1, aluA: 19.5, aluB: 23.8 }
    },
...
"514": { descrizione: "4 campi misti", campi: ["102", "101", "101", "102"], note: "4 campi" },
    ...
    // Riferimento: Listino Finstral EUR 2025/10 (verificato su conferma ordine)
    // 
...
"classic-line": { pvcA: 0, pvcB: 6.35, aluA: 10.7, aluB: 11.6 },      // codice 973K
"step-line": { pvcA: 0, pvcB: 6.24, aluA: 10.7, aluB: 11.2 },          // codice 974N
"slim-line": { pvcA: 1.94, pvcB: 8.87, aluA: 10.7, aluB: 11.6 },       // codice 970
"slim-line-cristal": { pvcA: 1.94, pvcB: 8.87, aluA: 10.7, aluB: 11.6 },
"slim-line-twin": { pvcA: 1.94, pvcB: 8.87, aluA: 10.7, aluB: 11.6 },
"nova-line": { pvcA: 5.51, pvcB: 11.6, aluA: 0, aluB: 0 },             // codice 953 - vetro esterno
"nova-line-30": { pvcA: 5.51, pvcB: 11.6, aluA: 0, aluB: 0 },
"nova-line-40": { pvcA: 5.51, pvcB: 11.6, aluA: 0, aluB: 0 },
"nova-line-plus": { pvcA: 23.1, pvcB: 29.3, aluA: 0, aluB: 0 },        // codice 941
"nova-line-twin": { pvcA: 5.51, pvcB: 11.6, aluA: 0, aluB: 0 }
    },
...
numAnteDaTipo = 3;  // 3 ante (composito)
    ...
    // 🆕 v8.45 FIX: Gestisce correttamente telaio con/senza "N" finale
    if (materiale === 'alluminio' && coloreAlluminio) {
// 🆕 v8.45 FIX: Determina codice rivestimento senza raddoppiare "N"
// Se telaio = "Z62N" → cerca "Z62N" (non "Z62NN")
// Se telaio = "961" → cerca "961N"
let codiceRiv = telaio;
if (!telaio.toUpperCase().endsWith('N')) {
    codiceRiv = telaio + "N";
}
const rivData = FINSTRAL_PREZZI.rivestimentiTelaiAlluminio?.[codiceRiv];
console.log(`🔬 v8.45: telaio="${telaio}" → codiceRiv="${codiceRiv}" → rivData=${rivData ? 'trovato' : 'NON trovato'}`);


if (rivData) {
    // ✅ v8.45: Usa valore pvcAlu per rivestimento alluminio esterno
    const valoreRiv = rivData.pvcAlu || rivData.pvcPvc || 20.9;  // Fallback a 20.9
    
    console.log(`🔬 Rivestimento ${codiceRiv}: €${valoreRiv}/ml × ${perimetro.toFixed(2)}ml`);
    ...
    console.log(`🔬 → supplementoTelaio (rivestimento) = €${risultato.dettaglio.supplementoTelaio}`);
} else {
    ...
    if (supplTelaio) {
        ...
        console.log(`🔬 Telaio ${telaio}: supplTelaio[${materiale}] = €${valoreSuppl}/ml × ${perimetro.toFixed(2)}ml (rivestimento ${telaio}N non trovato)`);
...
if (supplTelaio) {
    ...
    console.log(`🔬 Telaio ${telaio}: supplTelaio[${materiale}] = €${valoreSuppl}/ml × ${perimetro.toFixed(2)}ml`);
    risultato.dettaglio.supplementoTelaio = Math.round(valoreSuppl * perimetro * 100) / 100;
    console.log(`🔬 → supplementoTelaio = €${risultato.dettaglio.supplementoTelaio}`);
} else {
    risultato.dettaglio.supplementoTelaio = 0;
}
    }
...
if (supplProfilo) {
    // ✅ v7.79 FIX: Nova-line è SOLO PVC (ha vetro esterno, non alluminio)
    ...
    console.log(`🔬 Profilo ${antaNorm}: supplProfilo[${materialeProfilo}] = €${valoreProf}/ml × ${perimetroBattenti.toFixed(2)}ml (${numAnteDaTipo} ante)`);
...
risultato.dettaglio.supplementoMontante = 0;
    ...
    // ✅ v8.463 FIX: Supplemento colore PVC su PERIMETRO TELAIO (non battenti!)
    // Il colore PVC si applica al telaio, non alle ante
...
// ✅ v8.463: Usa perimetro TELAIO, non battenti!
risultato.dettaglio.supplementoColorePVC = Math.round(supplColore * perimetro * 100) / 100;
risultato.dettaglio.gruppoColorePVC = isGruppoB ? "B" : "A";
console.log(`🎨 Colore PVC ${isGruppoB ? 'B' : 'A'}: €${supplColore}/ml × ${perimetro.toFixed(2)}ml telaio = €${risultato.dettaglio.supplementoColorePVC}`);
    } else {
risultato.dettaglio.supplementoColorePVC = 0;
    }
    
    // ✅ v8.463 FIX: Supplemento colore alluminio su PERIMETRO TELAIO (non battenti!)
    if (materiale === "alluminio" && coloreAlluminio) {
...
if (supplAntaAlu) {
    ...
    // ✅ v8.463: Usa perimetro TELAIO, non battenti!
    risultato.dettaglio.supplementoColoreAlluminio = Math.round(supplAntaAlu[idxSuppl] * perimetro * 100) / 100;
    ...
    console.log(`🎨 Colore ALU ${gruppoAlu}: €${supplAntaAlu[idxSuppl]}/ml × ${perimetro.toFixed(2)}ml telaio = €${risultato.dettaglio.supplementoColoreAlluminio}`);
}
    } else {
...
console.log('✅ FINSTRAL_PREZZI v8.505 caricato (gruppi colore A/B)');
