...
// - Gruppo B = colori scuri (06, 13, 19, 55, 36, 46) → usa pvcB/aluB
//
// STRUTTURA SUPPLEMENTI TELAIO (€/ml perimetro telaio):
//   { pvcA, pvcB, aluA, aluB }
//   - pvcA/pvcB = supplemento profilo PVC interno
//   - aluA/aluB = supplemento rivestimento alluminio esterno
//
// STRUTTURA SUPPLEMENTI ANTA (€/ml perimetro anta):
//   { pvcA, pvcB, aluA, aluB }
...
//   Telaio 965: pvcB(13.1) + aluB(17.3) = 30.4 €/ml × perimetro
//   Anta Classic-line: pvcB(6.35) + aluB(11.6) = 17.95 €/ml × perimetro ante
//
// ============================================================================
...
window.FINSTRAL_PREZZI = {
    ...
    // ┌─────────────────────────────────────────────────────────────┐
    // │ Tipo         │ Altezza    │ Fattore │ Verifica              │
    // ├─────────────────────────────────────────────────────────────┤
    // │ 102 (fisso)  │ qualsiasi  │ 1.336   │ Pos.8 Alberto Papa    │
    // │ 101          │ H<=1600    │ 1.2075  │ (stimato)             │
    // │ 101          │ H>2200     │ 1.27    │ Pos.2,6 Ruggeri       │
    // │ 401          │ H<=1600    │ 1.2075  │ Pos.5,8 Ruggeri       │
    // │ 401          │ H>2200     │ 1.22    │ Pos.1 Ruggeri         │
    // │ 420/421      │ H<=1600    │ 1.21    │ Pos.3,4,7,9 Ruggeri   │
    // │ 420/421      │ H>2200     │ 1.22    │ (stimato)             │
    // └─────────────────────────────────────────────────────────────┘
...
// ═══════════════════════════════════════════════════════════════════
// 🆕 v8.484: TUTTI I TIPI CON PREZZI EUR 2025/10 → FATTORE 1.0!
// I prezzi nelle tabelle sono già quelli del listino EUR 2025/10
// NON applicare nessun fattore di correzione!
// ═══════════════════════════════════════════════════════════════════

// TIPO 102 (FISSO): prezzi EUR 2025/10 diretti
if (tipo === "102") {
    return 1.0;  // ✅ v8.484: Prezzi già corretti EUR 2025/10
}
...
return 1.22;
    ...
    fattoreCorrezione: 1.2075,  // Default per retrocompatibilità
...
prezzi: [
    ...
    // ═══════════════════════════════════════════════════════════════════
    // 🆕 v8.484: TIPO 102 (FISSO) - PREZZI EUR 2025/10 DIRETTI (NO FATTORE!)
    // Fonte: Listino Finstral EUR 2025/10 pag. 164
    // ⚠️ REGOLA: Usare SEMPRE prezzi EUR 2025/10 direttamente, fattore = 1.0
    // ═══════════════════════════════════════════════════════════════════
    tipo102: {
...
prezzi: [
    ...
    // ═══════════════════════════════════════════════════════════════════
    // 🆕 v8.482: TIPO 401 - PREZZI EUR 2025/10 DIRETTI (NO FATTORE!)
...
// ✅ v8.482: Corretto 1490→1480 come da listino EUR 2025/10
...
prezzi: [
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 v8.505: SUPPLEMENTI TELAIO CON GRUPPI COLORE A/B
    // Riferimento: Listino Finstral EUR 2025/10 (VERIFICATO)
    // 
    // STRUTTURA: pvcA/pvcB = supplemento PVC interno (€/ml)
    //            aluA/aluB = supplemento rivestimento ALU esterno (€/ml)
...
"965": { pvcA: 7.71, pvcB: 13.1, aluA: 15.0, aluB: 17.3 },   // pag.20-21 ✅ Verificato conferma Ruggeri
...
"951": { pvcA: 13.0, pvcB: 20.1, aluA: 19.5, aluB: 23.8 }    // stimato
    ...
    // ✅ v8.10: RIVESTIMENTO TELAIO ALLUMINIO (€/ml) - Listino EUR 2025/3 pag.10-11
    // Questo è il supplemento per il RIVESTIMENTO esterno in alluminio
    // Si applica INVECE di supplementiTelaio quando finituraEst = alluminio
    // ✅ v8.45 CORRETTO: Rivestimenti telaio alluminio da listino EUR 2025/10
    // Valori: PVC-PVC / PVC-ALU (€/ml)
...
"Z62N":  { pvcPvc: 18.3, pvcAlu: 20.9 },   // ✅ v8.45: Corretto da listino
"Z91N":  { pvcPvc: 18.3, pvcAlu: 22.0 }
    },

    // ✅ v8.10: Colori alluminio gruppo B (scuri/legno verniciato)
...
"L21", "L22", "L23", "L24", "L25", "L26", "L27", "L28", "L29", "L30"
    ...
    // SUPPLEMENTI VETRO (EUR/m²) - Listino 2025/3
    supplementiVetro: {
...
// 🆕 v8.45: Supplementi vetro satinato (da listino EUR 2025/10)
...
"satinato-bodysafe": 132.00  // Alias Bodysafe
    ...
    // ✅ v7.73: SOGLIA PORTA-FINESTRA (EUR/ml)
    // ✅ v8.10: Corretto prezzo da 53.40 a 54.50 (listino Finstral)
    supplementiSoglia: {
...
allargamento30K: 25.90   // +30mm alluminio
    ...
    // ✅ v8.466: SUPPLEMENTI FERRAMENTA SCORREVOLE (EUR/pezzo)
...
"710.3": 27.90,  // Scontri sicurezza multipli
    ...
    // ✅ v8.466: CONFIGURAZIONE TIPI SCORREVOLI
...
// 🆕 v8.478: Tipo 500 scorrevole parallela ha prezzo base ≈2× tabella 101
// 🆕 v8.478: supplementoFerramenta automatico per tipo 500 (€760 per meccanica scorrevole)
"500": { descrizione: "Scorrevole parallela", campi: ["101"], fattorePrezzo: 1.97, supplementoFerramenta: 760, note: "prezzo base × 1.97 + ferramenta automatica €760" },
...
"514": { descrizione: "4 campi misti", campi: ["102", "101", "101", "102"], note: "4 campi" },
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 v8.505: SUPPLEMENTO PROFILO ANTA CON GRUPPI COLORE A/B
    // Riferimento: Listino Finstral EUR 2025/10 (VERIFICATO)
    // 
    // STRUTTURA: pvcA/pvcB = supplemento PVC anta (€/ml perimetro anta)
    //            aluA/aluB = supplemento rivestimento ALU anta (€/ml)
    // ═══════════════════════════════════════════════════════════════════════════
    supplementiProfiloAnta: {
"classic-line": { pvcA: 0, pvcB: 6.35, aluA: 10.7, aluB: 11.6 },      // codice 973K - pag.117 ✅
...
"nova-line-twin": { pvcA: 6.35, pvcB: 11.9, aluA: 0, aluB: 0 }         // codice N989 - pag.139 EUR 2025/10 ✅ v8.511
    ...
    // Queste ante hanno prezzo a tabella dimensionale invece che €/ml
    tabelleAnteDimensionali: {
// Slim-line Cristal (codice 954) - pag.85
// [A mm][L mm] = €/pezzo supplemento
"slim-line-cristal": {
...
"nova-line-twin": {
    ...
    // ✅ v7.79: MONTANTE CENTRALE / NODO (EUR/pezzo) - Aggiornato con nodi specifici
...
// ✅ v7.79: Nodi specifici Nova-line
...
"N56": { pvcA: 46.10, aluA: 46.10 }    // Nova Twin vetro
    ...
    // ✅ v7.79: Mappa tipo anta → codice nodo
    nodiPerTipoAnta: {
...
"nova-line-twin": "N45"
    ...
    // ✅ v7.75: TAGLI SUI TELAI (EUR/ml) - Listino pag.22-28
    // I codici arrivano dal JSON (campo pos.infisso.taglio o tagli[])
    supplementiTagli: {
// Tagli standard €2.19/ml
...
"192": 5.22
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🆕 v8.505: GRUPPI COLORE PVC - PER LOOKUP
...
// 🆕 v8.505: I supplementi colore sono ORA INTEGRATI in supplementiTelaio e supplementiProfiloAnta
// Usare pvcA per gruppo A, pvcB per gruppo B
// La sezione "anta" è mantenuta per retrocompatibilità
anta: {
    ...
    // ✅ v7.64: SUPPLEMENTI COLORE ALLUMINIO (per rivestimento esterno)
    // Gruppo 1 = standard, 1H = decori legno, 2 = RAL (+284€/ordine), 3 = NCS/speciali (+1410€/ordine)
...
gruppo2: {
    ...
    costoPreparazione: 284  // €/ordine
},
gruppo3: {
    codici: ["NCS", "DB"],  // colori speciali
    costoPreparazione: 1410  // €/ordine
},
// Supplementi €/ml per anta alluminio [gruppo1, gruppo2-3]
anta: {
    ...
    // ✅ v7.64: DATABASE COLORI COMUNI (per lookup rapido)
    coloriComuni: {
...
"RAL 9005": { pvc: null, alluminio: "RAL", gruppo: "2" }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ✅ v7.77: FERRAMENTA / APERTURE / CERNIERE
...
"PFS": { 
    ...
    // CERNIERE: supplemento per cerniere a scomparsa (€ per unità)
    // Vista (4xx.0) = €0 incluse, Scomparsa (2xx.0) = supplemento
    supplementiCerniere: {
...
"estateInverno1": 14.5,    // 20-1 una anta (170→40mm)
...
// ════════════════════════════════════════════════════════════════════════════
// 🏠 DATABASE FIN-SLIDE - ALZANTI SCORREVOLI (Listino EUR 2025/10)
...
window.FINSLIDE_PREZZI = {
    ...
    // 🆕 v8.483: Tipologie Telaio corrette da configuratore Finstral
...
prezzi: {
    ...
    supplementiFerramenta: {
'83': { desc: 'Standard ≤250kg', prezzo: 0 },
'83F': { desc: 'Con stopper ≤250kg', prezzo: 12.2 },
'83.3': { desc: 'Sicurezza ≤250kg', prezzo: 79.6 },
'183': { desc: 'Pesante >250kg', prezzo: 95.7 },
'183.3': { desc: 'Pesante sicurezza', prezzo: 175.3 },
'84': { desc: 'Seconda anta ≤250kg', prezzo: 0 },
...
'ST': { desc: 'Soft-Stop', prezzo: 143 }
    ...
    // 🆕 v8.479: Maniglie FIN-Slide (listino EUR 2025/10 pag.68-79)
    maniglie: {
// Serie 2 - Alluminio (più comune)
'serie2': {
...
'serie12': {
    ...
    // 🆕 v8.480: Vetri FIN-Slide (listino EUR 2025/10 pag.87-99)
    // Prezzi supplemento per m² - Preisschema 2 (calcolo vetri VSG)
    vetri: {
// Max-Valor 3 con Multiprotect (triplo vetro 46mm) - più comune per HST
'Max-Valor3_46': {
...
// Plus-Valor 2 (doppio vetro) - più economico
'Plus-Valor2': {
    ...
    // 🆕 v8.483: Calcolo prezzo telaio con supplementi corretti
    calcolaPrezzoTelaio: function(tipoTelaio, L, H) {
// Determina griglia prezzi in base al tipo telaio
// 90M, 90N, 090A0 → usano griglia telaio 90 (base ALU-PVC 168)
// 38B, 38C → usano griglia telaio 38 (base ALU-PVC 157)
...
console.log(`🔧 Telaio ${tipoTelaio}: base €${prezzoBase} + suppl. ${perimetroM.toFixed(2)}ml × €${supplementoMl}/ml = €${supplementoTelaio}`);
...
console.log('🏠 FIN-SLIDE Database caricato - Dashboard v8.469');

// ============================================================================
// 🆕 v8.44: DATABASE FINSTRAL OTTOBRE 2025
// ============================================================================
...
// ═══════════════════════════════════════════════════════════════════════════
// ✅ v8.468: SISTEMA VALIDAZIONE PREVENTIVI
...
campi: {
    // 🔴 CRITICI - senza questi NO calcolo
    BRM_L: { 
        livello: 'critico', 
        label: 'Larghezza BRM',
    ...
    // 🟠 NECESSARI - warning + default, errore 5-15%
    telaio: { 
        livello: 'necessario', 
        label: 'Telaio',
    ...
    // 🟡 IMPORTANTI - info + default, errore 2-5%
    coloreEst: { 
        livello: 'importante', 
    ...
    // 🟢 OPZIONALI - nessun problema se mancano
...
valido: true,
errori: [],      // 🔴 Critici
warning: [],     // 🟠 Necessari mancanti
info: [],        // 🟡 Importanti mancanti
defaultUsati: [], // Campi dove è stato usato il default
margineErrore: 0,
affidabilita: 100
    };
...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
    <div style="background: #f0fdf4; border: 2px solid #22c55e; border-radius: 0.5rem; padding: 1rem; text-align: center;">
        <div style="font-size: 2rem; font-weight: 700; color: #22c55e;">✅ ${completi}</div>
    ...
    <div style="background: #fff7ed; border: 2px solid #f97316; border-radius: 0.5rem; padding: 1rem; text-align: center;">
        <div style="font-size: 2rem; font-weight: 700; color: #f97316;">🟠 ${conWarning}</div>
    ...
    <div style="background: #fef2f2; border: 2px solid #dc2626; border-radius: 0.5rem; padding: 1rem; text-align: center;">
        <div style="font-size: 2rem; font-weight: 700; color: #dc2626;">🔴 ${conErrori}</div>
    ...
    <div style="background: #f8fafc; border: 2px solid ${coloreAff}; border-radius: 0.5rem; padding: 1rem; text-align: center;">
        ...
        <div style="font-size: 0.875rem; color: #475569;">Affidabilità</div>
    </div>
</div>
    `;
...
<div style="background: #f9fafb; border-radius: 0.5rem; padding: 1rem; margin-bottom: 1rem;">
    <h4 style="margin: 0 0 1rem 0; color: #374151;">📋 Dettaglio Problemi</h4>
`;
...
posizioniConProblemi.forEach(pos => {
    pos.prodotti.filter(p => p.stato !== 'completo').forEach(prod => {
        ...
        prod.errori?.forEach(err => {
            html += `<div style="color: #dc2626; font-size: 0.875rem; margin-left: 1rem;">🔴 ${err.messaggio}</div>`;
        });
        
        // Warning
        prod.warning?.forEach(warn => {
            html += `<div style="color: #f97316; font-size: 0.875rem; margin-left: 1rem;">🟠 ${warn.messaggio}${warn.default ? ` → default: ${warn.default}` : ''}</div>`;
        });
        
        // Info
        prod.info?.forEach(info => {
            html += `<div style="color: #eab308; font-size: 0.875rem; margin-left: 1rem;">🟡 ${info.messaggio}${info.default ? ` → default: ${info.default}` : ''}</div>`;
        });
        
        if (prod.affidabilita < 100) {
            html += `<div style="color: #6b7280; font-size: 0.75rem; margin-left: 1rem; margin-top: 0.25rem;">📈 Affidabilità: ${prod.affidabilita}% (margine errore ±${prod.margineErrore}%)</div>`;
        }
...
<div style="background: #f0fdf4; border: 1px solid #22c55e; border-radius: 0.5rem; padding: 1.5rem; text-align: center;">
    ...
    <div style="font-size: 0.875rem; color: #15803d; margin-top: 0.25rem;">Il calcolo sarà accurato al 100%</div>
</div>
`;
    }
...
<div style="background: white; border-radius: 0.75rem; max-width: 700px; width: 90%; max-height: 85vh; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25);">
    <div style="background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: white; padding: 1rem 1.5rem; display: flex; justify-content: space-between; align-items: center;">
        <h3 style="margin: 0; font-size: 1.25rem;">📋 Validazione Preventivo</h3>
    ...
    <div style="padding: 1rem 1.5rem; background: #f9fafb; border-top: 1px solid #e5e7eb; display: flex; justify-content: flex-end; gap: 0.75rem;">
        ...
        ` : `
            ...
            <button id="btn-procedi-calcolo" style="padding: 0.625rem 1.25rem; background: ${hasWarning ? '#f97316' : '#22c55e'}; color: white; border: none; border-radius: 0.375rem; cursor: pointer; font-weight: 500;">
                ${hasWarning ? '⚠️ Procedi con warning' : '✅ Calcola preventivo'}
            </button>
        `}
    </div>
</div>
...
// ═══════════════════════════════════════════════════════════════════
// ✅ v8.467: FUNZIONE CENTRALIZZATA DETERMINAZIONE TIPO INFISSO
...
isScorrevole: false,
isFinSlide: false,         // 🆕 v8.472: Flag FIN-Slide/HST
isPortaFinestra: false,
numAnte: 1
    };
    
    // ✅ v8.468c: USA tipoInfissoAssociato per decidere F vs PF
    ...
    // 🆕 v8.472: PRIORITÀ 0 - FIN-SLIDE (Alzante Scorrevole / HST)
...
console.log(`🏠 determinaTipoInfisso: FIN-Slide "${codiceModello}" → HST ${codiceNumerico}`);
return risultato;
    }
    
    // PRIORITÀ 1: Scorrevoli da codiceModello
...
risultato.numAnte = 2;
console.log(`🚪 determinaTipoInfisso: codiceModello="${codiceModello}" → Scorrevole`);
return risultato;
    }
    
    // PRIORITÀ 2: Scorrevoli da ferramenta
...
risultato.isPortaFinestra = true;
risultato.numAnte = 2;
console.log(`🚪 determinaTipoInfisso: ferramenta1="${ferramenta1}" → Scorrevole ${tipoScorr}`);
return risultato;
    }
    
    // PRIORITÀ 3: Usa codiceModello per calcolo prezzo + tipoInfissoAssociato per F/PF
...
console.log(`📌 determinaTipoInfisso: codiceModello="${codiceModello}", tipoInfissoAssociato="${tipoInfissoAssociato}" → ${risultato.tipoVisualizzato}`);
return risultato;
    }
    
    // PRIORITÀ 4: Usa campo tipo se presente
    if (tipo) {
...
console.log(`📋 determinaTipoInfisso: tipo="${tipo}" → ${risultato.tipoFinstral}`);
return risultato;
    }
    
    // PRIORITÀ 5: Solo tipoInfissoAssociato senza altri dati
    risultato.tipoVisualizzato = `${prefisso}1`;
    risultato.tipoFinstral = '101';
    risultato.numAnte = 1;
    console.log(`📋 determinaTipoInfisso: tipoInfissoAssociato="${tipoInfissoAssociato}", H=${H_mm}mm → ${risultato.tipoVisualizzato}`);
    return risultato;
}
...
window.trovaPrezzoBaseFinstral = function(tipo, larghezza, altezza) {
    ...
    // 
    // REGOLA TIPO 420:
    // campo 1 = tipo 101 → prezzo base secondo tabella tipo 101
    // campo 2 = tipo 401 → prezzo base secondo tabella tipo 401
    ...
    // ✅ v8.462: Fattore variabile basato su TIPO e ALTEZZA
    ...
    // ═══════════════════════════════════════════════════════════════════
    // ✅ v8.466: GESTIONE TIPI SCORREVOLI (500, 501, 503, etc.)
...
for (let i = 0; i < numCampi; i++) {
    ...
    if (!tabella) {
        console.warn(`⚠️ Tabella tipo${tipoTabella} non trovata per campo ${i+1}`);
        continue;
    }
    ...
    if (risultato) {
        ...
        dettagliCampi.push(`Campo${i+1}(tipo${tipoTabella}): L=${larghezzaCampo} → €${risultato.prezzo}`);
    }
}

if (prezzoTotale > 0) {
    // 🆕 v8.478: Usa fattore specifico se definito, altrimenti default 1.236
    // Tipo 500 scorrevole parallela: fattore 1.97 (€1286/€653)
    ...
    console.log(`🚪 tipo${tipo} SCORREVOLE: L=${larghezza}, H=${altezza}`);
    dettagliCampi.forEach(d => console.log(`   ${d}`));
    console.log(`   Base EUR 2025/3: €${prezzoTotale}`);
    console.log(`   × fattore ${fattoreScorrevole} = €${prezzoCorretto} (EUR 2025/10)`);
    return prezzoCorretto;
} else {
    console.warn(`⚠️ tipo${tipo} SCORREVOLE: impossibile calcolare prezzo`);
    return null;
}
    }
...
if (ris101 && ris401) {
    ...
    console.log(`📊 tipo${tipo} COMPOSITO: L=${larghezza}, H=${altezza}`);
    console.log(`   Campo1 (tipo101): L=${L_campo1} → €${ris101.prezzo}`);
    console.log(`   Campo2 (tipo401): L=${L_campo2} → €${ris401.prezzo}`);
    console.log(`   Base EUR 2025/3: €${prezzoBase}`);
    console.log(`   × fattore ${fattore} = €${prezzoCorretto} (EUR 2025/10)`);
    return prezzoCorretto;
} else {
    console.warn(`⚠️ tipo${tipo}: impossibile calcolare prezzo composito`);
    return null;
}
    }
    ...
    if (!tabella) {
console.warn(`⚠️ Tabella tipo${tipo} non trovata`);
return null;
    }
...
console.log(`📊 trovaPrezzoBase tipo${tipo}: L=${larghezza}→col[${risultato.colIdx}], H=${altezza}→riga[${risultato.rowIdx}]`);
console.log(`   Base EUR 2025/3: €${risultato.prezzo} × ${fattore} = €${prezzoCorretto} (EUR 2025/10)`);
return prezzoCorretto;
    }
    return null;
}
...
window.calcolaPrezzoFinstral = function(config) {
    ...
    console.log(`🔬 trovaPrezzoBaseFinstral(tipo=${tipo}, L=${larghezza}, H=${altezza}) = ${prezzoBase}`);
    if (prezzoBase === null) {
risultato.errore = "Dimensioni fuori range per tipo " + tipo;
console.warn(`⚠️ ERRORE: ${risultato.errore}`);
return risultato;
    }
    ...
    // ✅ v8.462 FIX: Calcolo PERIMETRO BATTENTI per supplemento profilo anta
...
numAnteDaTipo = 3;  // 3 ante (composito)
    ...
    // ✅ v7.73: Normalizza tipo anta PRIMA di usarlo
    const antaNorm = tipoAnta.toLowerCase().replace(/\s+/g, '-');
    
    // ✅ v8.10: Supplemento telaio CON RIVESTIMENTO ALLUMINIO
    // 🆕 v8.505: Usa struttura gruppi colore A/B (pvcA/pvcB/aluA/aluB)
    ...
    // ✅ v8.462 FIX: Supplemento PROFILO ANTA su PERIMETRO BATTENTI (non finestra!)
    // ⚠️ FISSO (tipo 102): NON ha ante, quindi supplementoProfiloAnta = 0
    if (numAnteDaTipo === 0) {
console.log(`🔬 Tipo ${tipo} (FISSO): 0 ante → supplementoProfiloAnta = €0`);
...
if (supplProfilo) {
    ...
    // Nova-line è SOLO PVC (ha vetro esterno, no alluminio)
    if (materiale === 'alluminio' && !isNovaLine) {
        suppAlu = supplProfilo[keyAlu] ?? supplProfilo.alluminio ?? 0;
    }
    
    const totaleProf = suppPvc + suppAlu;
    console.log(`🔬 Profilo ${antaNorm} gruppo ${gruppoColore}: PVC €${suppPvc}/ml + ALU €${suppAlu}/ml × ${perimetroBattenti.toFixed(2)}ml (${numAnteDaTipo} ante)`);
    ...
    console.log(`🔬 → supplementoProfiloAnta = €${risultato.dettaglio.supplementoProfiloAnta}`);
} else {
    console.log(`🔬 Profilo ${antaNorm}: NON TROVATO in supplementiProfiloAnta`);
    risultato.dettaglio.supplementoProfiloAnta = 0;
}
    }
    }  // ✅ v8.462: Fine blocco else per fisso
    
    // ✅ v7.79 FIX: Supplemento MONTANTE MOBILE per tipo 401 (2 ante)
...
console.log(`🔬 Montante: tipo anta ${antaNorm} → nodo ${codiceNodo}`);
if (montanteData) {
    ...
    if (materiale === 'alluminio' && coloreAlluminio) {
        // ✅ v7.79: Determina gruppo alluminio (A, B, C, D)
        ...
        console.log(`🔬 Montante ${codiceNodo}[${gruppoAlu}] = €${risultato.dettaglio.supplementoMontante}`);
    } else {
    ...
    console.warn(`⚠️ Nodo ${codiceNodo} non trovato in supplementiMontante`);
...
risultato.dettaglio.supplementoMontante = 0;
    ...
    // 🆕 v8.505: Supplemento colore PVC ora INTEGRATO in pvcA/pvcB di telaio e anta
    // Non calcolare separatamente per evitare duplicazioni
    risultato.dettaglio.supplementoColorePVC = 0;
    // Il gruppoColorePVC è già stato impostato sopra
    
    // 🆕 v8.505: Supplemento colore alluminio ora INTEGRATO in aluA/aluB di telaio e anta
...
console.log(`🪟 Bancale ${config.bancaleTipo}: P=${config.bancaleProfondita}mm L=${lunghezzaBancale}mm → €${risultato.dettaglio.bancale.prezzo}`);
    }
...
// ✅ v8.466: Per tipo 503, aggiungi supplemento ferramenta 430 (anta apribile)
const configScorrevole = FINSTRAL_PREZZI.tipiScorrevoli?.[tipo];
if (configScorrevole?.ferramenta430) {
    ...
    console.log(`🚪 Supplemento ferramenta 430 (anta apribile): €${suppl430}`);
}

// 🆕 v8.478: Supplemento ferramenta automatico per tipo 500 (scorrevole parallela)
if (configScorrevole?.supplementoFerramenta && risultato.dettaglio.supplementoFerramenta === 0) {
    ...
    console.log(`🚪 Supplemento ferramenta automatico tipo ${tipo}: €${configScorrevole.supplementoFerramenta}`);
}
    } else {
risultato.dettaglio.supplementoFerramenta = 0;
    }
    
    // 🆕 v8.45: Supplemento Accessori (se configurato)
...
console.log('✅ FINSTRAL_PREZZI caricato - Listino EUR 2025/3 + Colori + Ferramenta/Cerniere');
...
// ═══════════════════════════════════════════════════════════════════════════
// 🆕 v8.45: DATABASE ACCESSORI FINSTRAL (da conferme ordine reali)
// ═══════════════════════════════════════════════════════════════════════════
...
console.log('✅ v8.45: FINSTRAL_FERRAMENTA_PREZZI e FINSTRAL_ACCESSORI_PREZZI caricati');
window.FINSTRAL_FERRAMENTA_PREZZI = FINSTRAL_FERRAMENTA_PREZZI;
window.FINSTRAL_ACCESSORI_PREZZI = FINSTRAL_ACCESSORI_PREZZI;
...
bianco: [
    [181, 186, 196, 202, 213, 220, 228, 236, 243, 250, 260, 269, 276, 283, 291, 382, 388, 396, 403, 410, 417, 424, 429, 438, 443, 452, 458],
    [188, 197, 207, 218, 225, 232, 243, 252, 261, 271, 280, 289, 297, 307, 316, 405, 416, 424, 430, 440, 450, 457, 465, 473, 486, 494, 502],
    [196, 208, 219, 228, 237, 247, 258, 269, 278, 290, 299, 309, 319, 329, 341, 429, 441, 451, 459, 469, 484, 493, 502, 513, 523, 530, 542],
    [202, 218, 228, 240, 250, 261, 275, 285, 296, 308, 319, 329, 343, 356, 366, 456, 467, 482, 494, 508, 521, 530, 546, 557, 568, 582, 596],
    [213, 226, 237, 249, 263, 277, 289, 300, 312, 327, 341, 352, 366, 378, 391, 485, 498, 510, 523, 533, 549, 560, 571, 585, 599, 613, 626]
],
scuri: [
    [185, 196, 202, 213, 222, 230, 240, 247, 258, 267, 276, 283, 294, 300, 309, 400, 409, 417, 426, 433, 445, 456, 465, 476, 488, 499, 509],
    [196, 207, 218, 226, 236, 245, 257, 267, 276, 285, 295, 304, 316, 326, 332, 426, 433, 445, 454, 463, 470, 484, 491, 500, 509, 517, 525],
    [202, 218, 226, 237, 249, 260, 274, 283, 294, 304, 317, 327, 340, 350, 360, 451, 460, 473, 487, 498, 508, 517, 526, 535, 547, 555, 565],
    [213, 226, 237, 250, 263, 276, 289, 299, 310, 326, 336, 349, 361, 375, 388, 480, 494, 507, 517, 527, 538, 551, 561, 571, 583, 596, 607],
    [224, 236, 249, 263, 277, 290, 301, 317, 329, 344, 358, 373, 386, 400, 413, 508, 522, 530, 547, 560, 573, 589, 603, 621, 632, 649, 661]
]
    },
...
righeA: [250, 350, 450, 550, 650],
bianco: [
    [188, 196, 207, 216, 225, 236, 243, 252, 261, 272, 280, 290, 299, 308, 317, 409, 416, 426, 432, 442, 452, 459, 468, 480, 488, 498, 507],
    [196, 207, 217, 226, 238, 247, 258, 269, 280, 290, 299, 310, 322, 332, 343, 430, 442, 454, 463, 470, 484, 491, 500, 509, 517, 525, 532],
    [202, 217, 228, 238, 249, 261, 275, 286, 296, 308, 318, 331, 344, 358, 372, 457, 469, 484, 497, 509, 522, 532, 547, 558, 570, 583, 598],
    [210, 225, 238, 249, 263, 277, 289, 300, 315, 328, 341, 356, 368, 382, 396, 482, 493, 512, 525, 538, 553, 566, 581, 596, 609, 626, 641],
    [219, 236, 247, 261, 277, 290, 304, 325, 340, 356, 372, 387, 402, 417, 430, 510, 526, 540, 553, 565, 579, 593, 607, 621, 631, 646, 658]
],
scuri: [
    [196, 207, 218, 228, 238, 249, 260, 272, 283, 291, 301, 312, 325, 333, 346, 438, 445, 457, 465, 473, 486, 494, 502, 512, 521, 527, 535],
    [202, 217, 228, 240, 252, 264, 277, 289, 299, 310, 325, 333, 347, 359, 372, 460, 473, 489, 500, 510, 521, 528, 538, 550, 558, 567, 579],
    [216, 226, 240, 252, 266, 278, 291, 304, 317, 331, 344, 360, 377, 391, 407, 491, 505, 528, 544, 557, 570, 585, 600, 616, 630, 646, 660],
    [224, 237, 250, 264, 280, 294, 308, 331, 349, 364, 382, 399, 414, 429, 447, 515, 529, 546, 558, 571, 587, 602, 619, 631, 647, 660, 672],
    [231, 245, 263, 278, 294, 308, 325, 341, 357, 372, 387, 403, 423, 439, 456, 535, 557, 573, 591, 605, 624, 640, 654, 668, 686, 702, 717]
]
    },
...
righeA: [250, 350, 450, 550, 650],
bianco: [
    [199, 213, 224, 236, 245, 257, 267, 278, 290, 299, 310, 322, 332, 346, 357, 445, 457, 465, 482, 494, 508, 521, 530, 546, 557, 568, 582],
    [213, 225, 237, 247, 261, 275, 286, 297, 309, 322, 332, 347, 359, 372, 385, 473, 489, 500, 513, 525, 537, 551, 563, 574, 589, 602, 616],
    [224, 237, 249, 263, 277, 290, 301, 316, 329, 344, 358, 372, 386, 399, 411, 507, 517, 529, 546, 558, 571, 587, 602, 619, 631, 647, 660],
    [232, 247, 263, 277, 291, 307, 318, 333, 350, 363, 382, 396, 409, 424, 439, 530, 547, 560, 574, 593, 607, 625, 641, 656, 670, 687, 703],
    [244, 260, 277, 291, 307, 325, 340, 356, 372, 387, 402, 417, 430, 450, 463, 560, 574, 593, 607, 625, 641, 656, 670, 687, 703, 719, 734]
],
scuri: [
    [216, 226, 240, 252, 264, 278, 291, 301, 316, 329, 343, 357, 367, 383, 396, 489, 500, 513, 526, 540, 554, 567, 582, 598, 613, 627, 644],
    [226, 240, 254, 267, 283, 296, 309, 325, 340, 352, 366, 383, 396, 410, 424, 517, 529, 546, 558, 571, 587, 602, 619, 631, 647, 660, 672],
    [238, 252, 267, 285, 297, 315, 329, 346, 360, 377, 391, 407, 423, 438, 451, 547, 560, 574, 593, 607, 625, 641, 656, 670, 687, 703, 719],
    [250, 266, 285, 299, 316, 331, 349, 364, 385, 399, 414, 429, 447, 460, 482, 574, 594, 608, 626, 644, 658, 671, 688, 704, 721, 735, 751],
    [263, 280, 297, 315, 332, 350, 366, 387, 403, 423, 439, 456, 471, 494, 510, 605, 625, 645, 659, 672, 689, 707, 722, 736, 753, 769, 786]
]
    },
...
righeA: [250, 350, 450, 550, 650],
gruppo1: [
    [257, 275, 294, 312, 328, 347, 363, 380, 396, 412, 428, 443, 459, 472],
    [294, 312, 335, 353, 372, 394, 413, 432, 449, 468, 486, 504, 522, 538],
    [335, 357, 382, 404, 426, 449, 472, 493, 514, 538, 559, 580, 602, 622],
    [372, 398, 426, 452, 478, 504, 528, 554, 579, 605, 630, 654, 680, 703],
    [410, 440, 471, 502, 530, 561, 589, 619, 647, 675, 705, 732, 761, 788]
],
gruppo23: [
    [387, 414, 443, 472, 495, 523, 547, 572, 595, 621, 645, 667, 690, 712],
    [443, 472, 504, 532, 561, 594, 623, 651, 677, 707, 733, 760, 788, 811],
    [504, 538, 575, 609, 642, 677, 712, 744, 775, 810, 842, 873, 906, 935],
    [561, 599, 642, 682, 720, 760, 795, 835, 872, 911, 948, 985, 1024, 1058],
    [619, 662, 710, 755, 798, 845, 888, 932, 975, 1017, 1060, 1102, 1146, 1186]
]
    },
...
righeA: [250, 350, 450, 550, 650],
gruppo1: [
    [298, 321, 345, 370, 391, 414, 437, 458, 481, 502, 524, 544, 566, 585],
    [345, 370, 398, 424, 447, 474, 499, 523, 547, 571, 595, 619, 643, 665],
    [398, 426, 458, 487, 517, 547, 576, 605, 633, 664, 693, 721, 750, 777],
    [447, 481, 517, 551, 584, 619, 651, 686, 719, 753, 787, 818, 852, 884],
    [499, 537, 579, 619, 655, 694, 731, 770, 807, 846, 884, 919, 957, 993]
],
gruppo23: [
    [449, 484, 521, 559, 590, 625, 658, 690, 724, 756, 789, 818, 851, 881],
    [521, 559, 600, 639, 674, 715, 751, 787, 824, 861, 896, 931, 968, 1000],
    [600, 642, 689, 733, 777, 824, 868, 911, 954, 999, 1043, 1085, 1129, 1169],
    [674, 724, 777, 830, 879, 932, 982, 1032, 1083, 1134, 1185, 1233, 1283, 1331],
    [751, 808, 870, 931, 988, 1047, 1102, 1160, 1216, 1274, 1330, 1384, 1441, 1495]
]
    },
...
// ✅ v8.11: Alias per codici OpenPorte
...
console.log('✅ FINSTRAL_CASSONETTI_PREZZI caricato - Listino EUR 2025/3 (pag. 141-146)');
...
console.log(`📦 Colore cassonetto "${coloreCass}" → BIANCO (codice ${codice})`);
return 'bianco';
    } else if (codiciScuri.includes(codice)) {
console.log(`📦 Colore cassonetto "${coloreCass}" → SCURI (codice ${codice})`);
return 'scuri';
    } else {
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔧 v8.09: DATABASE CODICI FINSTRAL (codiceModello + ferramenta)
...
// ✅ v8.70: Export globali per costanti usate da app.js
window.FINSTRAL_CODICI = FINSTRAL_CODICI;
...
console.log('✅ FINSTRAL_PREZZI v8.511 caricato');

// ============================================================================
// ✅ v8.511: HELPER FINSTRAL (merge da finstral-module.js)
// ============================================================================
...
const COSTI_EXTRA = {
    posa_per_pezzo: 50,
    smontaggio_per_pezzo: 30
};
if (typeof window !== 'undefined') {
    window.COSTI_EXTRA = COSTI_EXTRA;
}

console.log('✅ FINSTRAL helper merge da finstral-module.js completato');
