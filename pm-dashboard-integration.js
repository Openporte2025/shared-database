...
window.createPMProject = async function() {
    ...
    if (!name) {
        if (typeof showAlert === 'function') showAlert('error', '❌ Nome obbligatorio');
        document.getElementById('pmProjectName').focus();
        return;
    }
    if (!client) {
        if (typeof showAlert === 'function') showAlert('error', '❌ Cliente obbligatorio');
    ...
    if (typeof PROJECT_MANAGER !== 'undefined') {
        project = PROJECT_MANAGER.createProject(name, client, window.githubProjects || []);
    } else {
        // Fallback
        const year = new Date().getFullYear();
        const num = (window.githubProjects?.length || 0) + 1;
        const projectId = `${year}P${String(num).padStart(3, '0')}`;
        const now = new Date().toISOString();
        project = {
            id: projectId, name, client, customerName: client,
            clientData: { nome: client },
            prodotti: { infissi: true },
            positions: [], posizioni: [],
            createdAt: now,
            metadata: { version: 1, created: now, updated: now }
        };
    }
    
    // Aggiungi dati extra
    project.clientData.telefono = document.getElementById('pmProjectPhone').value.trim();
    project.clientData.email = document.getElementById('pmProjectEmail').value.trim();
    project.clientData.indirizzo = document.getElementById('pmProjectAddress').value.trim();
    project.customerPhone = project.clientData.telefono;
    project.customerEmail = project.clientData.email;
    project.customerAddress = project.clientData.indirizzo;
    ...
    // Salva su GitHub - usa la funzione della dashboard
    try {
        // Prepara il progetto con tutti i campi necessari
        const projectToSave = {
            ...project,
            posizioni: project.positions || [],
            configPreventivo: {},
            vociExtra: [],
            changeLog: [{
        ...
        // Imposta riferimenti per salvaProgettoSuGitHub
        window._githubProjectRef = {
            id: project.id,
            rawData: projectToSave
        };
        window.currentData = projectToSave;
        window.projectData = projectToSave;
        
        console.log('📦 Dati preparati:', projectToSave);
        console.log('📦 _githubProjectRef:', window._githubProjectRef);
        
        if (typeof salvaProgettoSuGitHub === 'function') {
            console.log('💾 Salvando via salvaProgettoSuGitHub...');
        ...
        } else if (typeof GITHUB_SYNC !== 'undefined' && typeof GITHUB_SYNC.salvaProgetto === 'function') {
            console.log('💾 Salvando via GITHUB_SYNC.salvaProgetto...');
            await GITHUB_SYNC.salvaProgetto(projectToSave);
            alert(`✅ Progetto "${name}" creato con successo!`);
            ...
            console.error('❌ Nessuna funzione di salvataggio disponibile');
            console.log('Funzioni disponibili:', {
                salvaProgettoSuGitHub: typeof salvaProgettoSuGitHub,
                GITHUB_SYNC: typeof GITHUB_SYNC
            });
            alert('⚠️ Funzione salvataggio non trovata');
        }
    } catch (err) {
