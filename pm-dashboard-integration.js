...
window.createPMProject = async function() {
    ...
    if (typeof GITHUB_SYNC !== 'undefined' && GITHUB_SYNC.hasToken()) {
        if (typeof showAlert === 'function') showAlert('info', '💾 Salvataggio...');
        try {
            await GITHUB_SYNC.salvaProgetto(project);
            if (typeof showAlert === 'function') showAlert('success', `✅ Progetto "${name}" creato!`);
            if (typeof loadProjectsFromGitHub === 'function') {
                await loadProjectsFromGitHub();
            }
        } catch (err) {
            console.error(err);
            if (typeof showAlert === 'function') showAlert('error', '❌ Errore: ' + err.message);
        }
    } else {
        if (typeof showAlert === 'function') showAlert('warning', '⚠️ GitHub non configurato');
    }
