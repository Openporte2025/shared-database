...
async function saveNewProjectToGitHub(project) {
    // Cerca configurazione GitHub
    const config = window.GITHUB_DASHBOARD_CONFIG || window.GITHUB_CONFIG;
    if (!config || !config.token) {
        throw new Error('Token GitHub non configurato');
    }
    
    const owner = config.owner || 'openporte2025';
    const repo = config.repo || 'progetti-open';
    const filename = `progetti/progetto-${project.id}.json`;
    
    console.log(`📤 Salvando nuovo progetto: ${filename}`);
