...
async function editorSave() {
    ...
    try {
        ...
        if (syncSuccess) {
            ...
            // 🔧 v1.3.1: RICARICA COMPLETA UI dopo salvataggio
            // Questo garantisce che la vista sia sempre sincronizzata con i dati salvati
            setTimeout(() => {
                if (typeof renderVistaPosizioniUfficio === 'function') {
                    console.log('🔄 v1.3.1: Ricarica completa Vista Posizioni...');
                    renderVistaPosizioniUfficio(currentData);
                    
                    // Riseleziona la posizione corrente
                    if (typeof selectPosition === 'function') {
                        const newIndex = (currentData.posizioni || currentData.positions || [])
                            .findIndex(p => p.id === posId);
                        if (newIndex >= 0) {
