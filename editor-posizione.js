...
// v1.3.3: RICARICA DA GITHUB dopo salvataggio - metodo più sicuro
// ============================================================================

const EDITOR_VERSION = '1.3.3';
...
async function editorSave() {
    ...
    try {
        ...
        if (syncSuccess) {
            showEditorToast('✅ Salvato! Ricarico dati...');
            console.log(`✅ Posizione ${posId} sincronizzata su GitHub`);
            
            // 🔧 v1.3.3: RICARICA PROGETTO DA GITHUB dopo salvataggio
            // Questo è il metodo più sicuro - evita problemi di sincronizzazione
            setTimeout(async () => {
                try {
                    // Ottieni ID progetto
                    const projectId = currentData.id || currentData.commessa_id || 
                                     window._githubProjectRef?.id;
                    
                    if (projectId && typeof GITHUB_SYNC !== 'undefined') {
                        console.log('🔄 v1.3.3: Ricarico progetto da GitHub:', projectId);
                        
                        const freshData = await GITHUB_SYNC.caricaProgetto(projectId);
                        
                        if (freshData) {
                            // Aggiorna currentData con dati freschi
                            Object.assign(currentData, freshData);
                            
                            // Sincronizza positions/posizioni
                            if (freshData.positions && !freshData.posizioni) {
                                currentData.posizioni = freshData.positions;
                            }
                            if (freshData.posizioni && !freshData.positions) {
                                currentData.positions = freshData.posizioni;
                            }
                            
                            // Ri-renderizza la vista completa
                            if (typeof renderVistaPosizioniUfficio === 'function') {
                                renderVistaPosizioniUfficio(currentData);
                                console.log('✅ v1.3.3: Vista ricaricata da GitHub');
                                
                                // Riseleziona la posizione modificata
                                const positions = currentData.posizioni || currentData.positions || [];
                                const newIndex = positions.findIndex(p => p.id === posId);
                                if (newIndex >= 0 && typeof selectPosition === 'function') {
                                    selectPosition(newIndex);
                                }
                            }
                            
                            showEditorToast('✅ Dati aggiornati!');
                        }
                    } else {
                        // Fallback: almeno aggiorna la lista
                        console.log('⚠️ v1.3.3: Nessun GITHUB_SYNC, aggiorno solo lista');
                        if (typeof renderPositionsList === 'function') {
                            renderPositionsList();
                        }
                        if (typeof renderPositionDetail === 'function') {
                            ...
                            if (idx >= 0) renderPositionDetail(idx);
                        }
                    }
                } catch (reloadErr) {
                    console.error('❌ v1.3.3: Errore ricarica:', reloadErr);
                    showEditorToast('⚠️ Salvato ma errore ricarica. Ricarica pagina.');
                }
            }, 500);
