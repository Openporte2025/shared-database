...
// ============================================================================
// SALVATAGGIO - v1.2.3 con fix riferimenti (usa ID, non indice)
// ============================================================================
...
async function editorSave() {
    ...
    // 🔧 v1.2.3: Crea UNA sola copia profonda da usare ovunque
    const savedPosition = JSON.parse(JSON.stringify(editorState.currentPosition));
    
    // 1. Aggiorna la posizione in currentData (trova per ID)
    const currentDataIndex = currentData.positions.findIndex(p => p.id === posId);
    if (currentDataIndex >= 0) {
        currentData.positions[currentDataIndex] = JSON.parse(JSON.stringify(savedPosition));
        console.log(`✅ currentData.positions[${currentDataIndex}] aggiornato`);
    } else {
        console.warn(`⚠️ Posizione ${posId} non trovata in currentData.positions`);
    }
    
    // 2. Marca come modificato manualmente (protezione auto-sync)
    if (currentData.positions[currentDataIndex]?.infisso) {
        currentData.positions[currentDataIndex].infisso.manuallyEdited = true;
    }
    ...
    // 5. 🔧 FIX v1.2.3: Aggiorna variabili Dashboard usando ID
    try {
        // Aggiorna allPositionsData (trova per ID)
        if (typeof allPositionsData !== 'undefined' && Array.isArray(allPositionsData)) {
            const allPosIndex = allPositionsData.findIndex(p => p.id === posId);
            if (allPosIndex >= 0) {
                allPositionsData[allPosIndex] = JSON.parse(JSON.stringify(savedPosition));
                console.log(`✅ allPositionsData[${allPosIndex}] aggiornato`);
            }
        }
        
        // Aggiorna filteredPositions (trova per ID)
        if (typeof filteredPositions !== 'undefined' && Array.isArray(filteredPositions)) {
            const filteredIndex = filteredPositions.findIndex(p => p.id === posId);
            if (filteredIndex >= 0) {
                filteredPositions[filteredIndex] = JSON.parse(JSON.stringify(savedPosition));
                console.log(`✅ filteredPositions[${filteredIndex}] aggiornato`);
            }
        }
        
        // Aggiorna projectData.posizioni (trova per ID)
        if (typeof projectData !== 'undefined' && projectData) {
            if (projectData.posizioni && Array.isArray(projectData.posizioni)) {
                const projPosIndex = projectData.posizioni.findIndex(p => p.id === posId);
                if (projPosIndex >= 0) {
                    projectData.posizioni[projPosIndex] = JSON.parse(JSON.stringify(savedPosition));
                    console.log(`✅ projectData.posizioni[${projPosIndex}] aggiornato`);
                }
            }
            if (projectData.positions && Array.isArray(projectData.positions)) {
                const projPosIndex2 = projectData.positions.findIndex(p => p.id === posId);
                if (projPosIndex2 >= 0) {
                    projectData.positions[projPosIndex2] = JSON.parse(JSON.stringify(savedPosition));
                    console.log(`✅ projectData.positions[${projPosIndex2}] aggiornato`);
                }
            }
        }
