// ============================================================================
// EDITOR POSIZIONE - Dashboard Rilievi v8.497
// ============================================================================
...
// v1.2.1: Fix matching case-insensitive + fallback valori custom
// v1.2.2: Fix aggiornamento UI Dashboard (allPositionsData, filteredPositions)
// v1.2.3: Fix riferimenti - usa ID posizione invece di indice
// v1.3.0: Logica business allineata con app rilievo (sincro colori, auto-cerniere)
// v1.3.1: Ricarica completa UI dopo salvataggio (fix perdita prodotti)
// v1.3.2: Fix sincronizzazione positions↔posizioni + aggiornamento diretto variabili
// ============================================================================
        ...
        if (field.type === 'checkbox') {
            ...
            if (field.type === 'select') {
                ...
                options.forEach(opt => {
                    ...
                    // Match esatto case-insensitive O match parziale (contiene)
...
function editorFieldChanged(element) {
    ...
    // 🔧 v1.3.0: Usa DATA_MANAGER per aggiornamenti (stessa logica app rilievo)
    const posId = editorState.currentPosition.id;
    
    if (tab === 'posizione') {
        // Aggiorna campo base posizione
        editorState.currentPosition[key] = value;
        
        // Sincronizza quantità ai prodotti (come fa app rilievo)
        if (key === 'quantita') {
            const qta = value || '1';
            if (editorState.currentPosition.infisso) editorState.currentPosition.infisso.qta = qta;
            if (editorState.currentPosition.persiana) editorState.currentPosition.persiana.qta = qta;
            if (editorState.currentPosition.tapparella) editorState.currentPosition.tapparella.qta = qta;
            if (editorState.currentPosition.zanzariera) editorState.currentPosition.zanzariera.qta = qta;
            if (editorState.currentPosition.cassonetto) editorState.currentPosition.cassonetto.qta = qta;
        ...
        // Aggiorna prodotto (infisso, persiana, tapparella, etc.)
        if (!editorState.currentPosition[tab]) {
            editorState.currentPosition[tab] = {};
        }
        editorState.currentPosition[tab][key] = value;
        
        // 🔧 v1.3.0: Applica logica business come app rilievo
        if (tab === 'infisso') {
            const inf = editorState.currentPosition.infisso;
        ...
        // Persiana: auto tipoInfissoAssociato
        if (tab === 'persiana' && key === 'tipo') {
            const per = editorState.currentPosition.persiana;
            if (value && value.includes('PF')) {
...
// ============================================================================
// SALVATAGGIO - v1.3.0 con fix critico positions/posizioni
// ============================================================================
...
async function editorSave() {
    ...
    // 🔧 v1.3.0: Crea UNA sola copia profonda da usare ovunque
    const savedPosition = JSON.parse(JSON.stringify(editorState.currentPosition));
    
    // 1. 🔧 FIX CRITICO: Aggiorna ENTRAMBI i campi (positions E posizioni)
    //    La dashboard usa "posizioni", il JSON originale ha "positions"
    
    // Aggiorna currentData.positions (per salvataggio su GitHub)
    if (currentData.positions && Array.isArray(currentData.positions)) {
        const idx = currentData.positions.findIndex(p => p.id === posId);
        if (idx >= 0) {
            currentData.positions[idx] = JSON.parse(JSON.stringify(savedPosition));
            console.log(`✅ currentData.positions[${idx}] aggiornato`);
        }
    }
    
    // Aggiorna currentData.posizioni (per visualizzazione dashboard)
    if (currentData.posizioni && Array.isArray(currentData.posizioni)) {
        const idx = currentData.posizioni.findIndex(p => p.id === posId);
        if (idx >= 0) {
            currentData.posizioni[idx] = JSON.parse(JSON.stringify(savedPosition));
            console.log(`✅ currentData.posizioni[${idx}] aggiornato`);
        }
    }
    
    // 2. Marca come modificato manualmente
    const updatedPos = (currentData.positions || currentData.posizioni || []).find(p => p.id === posId);
    if (updatedPos?.infisso) {
        updatedPos.infisso.manuallyEdited = true;
    }
    ...
    // 5. 🔧 FIX: Aggiorna TUTTE le variabili della Dashboard usando ID
    try {
        // Aggiorna allPositionsData
        if (typeof allPositionsData !== 'undefined' && Array.isArray(allPositionsData)) {
            const idx = allPositionsData.findIndex(p => p.id === posId);
            if (idx >= 0) {
                allPositionsData[idx] = JSON.parse(JSON.stringify(savedPosition));
                console.log(`✅ allPositionsData[${idx}] aggiornato`);
            }
        }
        
        // Aggiorna filteredPositions
        if (typeof filteredPositions !== 'undefined' && Array.isArray(filteredPositions)) {
            const idx = filteredPositions.findIndex(p => p.id === posId);
            if (idx >= 0) {
                filteredPositions[idx] = JSON.parse(JSON.stringify(savedPosition));
                console.log(`✅ filteredPositions[${idx}] aggiornato`);
            }
        }
        
        // Aggiorna projectData (sia posizioni che positions)
        if (typeof projectData !== 'undefined' && projectData) {
            if (projectData.posizioni && Array.isArray(projectData.posizioni)) {
                const idx = projectData.posizioni.findIndex(p => p.id === posId);
                if (idx >= 0) {
                    projectData.posizioni[idx] = JSON.parse(JSON.stringify(savedPosition));
                    console.log(`✅ projectData.posizioni[${idx}] aggiornato`);
                }
            }
            if (projectData.positions && Array.isArray(projectData.positions)) {
                const idx = projectData.positions.findIndex(p => p.id === posId);
                if (idx >= 0) {
                    projectData.positions[idx] = JSON.parse(JSON.stringify(savedPosition));
                    console.log(`✅ projectData.positions[${idx}] aggiornato`);
                }
            }
        }
        
        // Trova indice in filteredPositions per il render
        let renderIndex = posIndex;
        if (typeof filteredPositions !== 'undefined' && Array.isArray(filteredPositions)) {
            const idx = filteredPositions.findIndex(p => p.id === posId);
        ...
        if (typeof renderPositionDetail === 'function') {
            renderPositionDetail(renderIndex);
            console.log(`✅ renderPositionDetail(${renderIndex}) chiamato`);
        }
    ...
    try {
        ...
        if (syncSuccess) {
            showEditorToast('✅ Salvato e sincronizzato!');
            console.log(`✅ Posizione ${posId} sincronizzata su GitHub`);
            
            // 🔧 v1.3.2: SINCRONIZZA positions ↔ posizioni prima del render
            // La dashboard legge da "posizioni", il JSON originale potrebbe avere "positions"
            if (currentData.positions && !currentData.posizioni) {
                currentData.posizioni = currentData.positions;
                console.log('🔄 v1.3.2: Creato alias posizioni → positions');
            }
            if (currentData.posizioni && !currentData.positions) {
                currentData.positions = currentData.posizioni;
                console.log('🔄 v1.3.2: Creato alias positions → posizioni');
            }
            
            // Assicurati che entrambi puntino agli stessi dati aggiornati
            const updatedPositions = currentData.posizioni || currentData.positions || [];
            currentData.posizioni = updatedPositions;
            currentData.positions = updatedPositions;
            
            // Debug: verifica che i prodotti siano presenti
            const debugPos = updatedPositions.find(p => p.id === posId);
            console.log('🔍 v1.3.2 DEBUG - Posizione salvata:', {
                id: posId,
                ambiente: debugPos?.ambiente,
                hasInfisso: !!debugPos?.infisso,
                infissoAzienda: debugPos?.infisso?.azienda
            });
            
            // 🔧 v1.3.2: AGGIORNA DIRETTAMENTE le variabili globali della dashboard
            if (typeof allPositionsData !== 'undefined') {
                // Svuota e riempi con i nuovi dati (mantiene il riferimento)
                allPositionsData.length = 0;
                updatedPositions.forEach(p => allPositionsData.push(p));
                console.log('✅ v1.3.2: allPositionsData aggiornato:', allPositionsData.length);
            }
            
            if (typeof filteredPositions !== 'undefined') {
                filteredPositions.length = 0;
                updatedPositions.forEach(p => filteredPositions.push(p));
                console.log('✅ v1.3.2: filteredPositions aggiornato:', filteredPositions.length);
            }
            
            // Render UI
            setTimeout(() => {
                // Rendi lista posizioni
                if (typeof renderPositionsList === 'function') {
                    renderPositionsList();
                    console.log('✅ v1.3.2: renderPositionsList chiamato');
                }
                
                // Trova l'indice della posizione modificata
                const newIndex = updatedPositions.findIndex(p => p.id === posId);
                console.log('🔍 v1.3.2: Posizione modificata è all\'indice', newIndex);
                
                // Rendi dettaglio posizione
                if (newIndex >= 0 && typeof renderPositionDetail === 'function') {
                    renderPositionDetail(newIndex);
                    console.log('✅ v1.3.2: renderPositionDetail chiamato per indice', newIndex);
                }
                
                // Seleziona la posizione nella lista
                if (newIndex >= 0 && typeof selectPosition === 'function') {
                    // Aggiorna currentPositionIndex
